...
public abstract class SpawnZone : GameLevelObject
{
    ...
    public virtual void SpawnShapes()
    {
        ...
        {
            CreateSateliteFor(shape, spawnConfig.satellite.uniformLifecycles?lifecycleDurations : spawnConfig.lifecycle.RandomDurations);
        }
    ...
    private Vector3 GetDirectionVector(SpawnConfiguration.MovementDirection direction, Transform t)
    {
        switch (direction)
        {
            ...
            case SpawnConfiguration.MovementDirection.Inward:
                return (transform.position - t.localPosition).normalized;
    ...
    private void SetupOscilation(Shape shape)
    {
    ...
    void CreateSateliteFor(Shape focalShape, Vector3 lifecycleDurations)
    {
        int factoryIndex = Random.Range(0, spawnConfig.factories.Length);
        Shape shape = spawnConfig.factories[factoryIndex].GetRandom();
        shape.gameObject.layer = gameObject.layer;
