...
public class GravityBox : GravitySource
{
    ...
    private float innerDistance = 3f, innerFalloffDistance = 6f;
    [SerializeField, Min(0f)]
    float outerDistance = 3f, outerFalloffDistance = 6f;

    float innerFallofFactor, outerFalloffFactor;
    ...
    private void OnValidate()
    {
        ...
        innerFallofFactor = 1f / (innerFalloffDistance - innerDistance);
    ...
    float GetGravityComponent(float coordinate, float distance)
    {
        ...
        {
            g *= 1f - (distance - innerDistance) * innerFallofFactor;
        }
